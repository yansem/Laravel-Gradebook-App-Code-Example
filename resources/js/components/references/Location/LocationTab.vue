<template>
    <location-form v-if="isShowModal"
...
export default {
    ...
    methods: {
        async getElements(params) {
            ...
            await axios.get('/api/v1/references/locations', {params})
                .then(res => {
        ...
        async storeElement() {
            await axios.post('/api/v1/references/locations', this.element)
                .then(res => {
                    this.modalObj.hide()
        ...
        async updateElement() {
            await axios.patch(`/api/v1/references/locations/${this.element.id}`, this.element)
                .then(res => {
                    this.modalObj.hide()
                    this.getElements({page: this.page, ...this.filterParams})
                    this.$notify({title: this.saveMsg, type: 'success'})
        ...
        async deleteElement(id) {
            await axios.delete(`/api/v1/references/locations/${id}`)
                .then(res => {
        ...
        async restoreElement(id) {
            await axios.get(`/api/v1/references/locations/restore/${id}`)
                .then(res => {
                    this.getElements({page: this.page, ...this.filterParams})
        ...
        async showModalForm(title, action, element = null) {
            ...
            await this.$nextTick()
            this.modal = document.getElementById('locationForm')
            this.modalObj = new Modal(this.modal)
            this.formTitle = title + ' аудиторию'
