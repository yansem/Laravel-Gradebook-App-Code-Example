                ...
                <div class="table-responsive table-responsive-reference">
                    ...
                    <table class="table table-hover table-bordered table-sm componentTable bg-white">
                        ...
                        <tr>
                            ...
                            <td><input v-model.trim="filters.value.value" type="text" class="form-control"></td>
...
export default {
    ...
    methods: {
        async getElements(params) {
            ...
            await axios.get('/api/v1/references/grades', {params})
                .then(res => {
        ...
        async storeElement() {
            await axios.post('/api/v1/references/grades', this.element)
                .then(res => {
                    this.modalObj.hide()
        ...
        async updateElement() {
            await axios.patch(`/api/v1/references/grades/${this.element.id}`, this.element)
                .then(res => {
                    this.modalObj.hide()
                    this.getElements({page: this.page, ...this.filterParams})
                    this.$notify({title: this.saveMsg, type: 'success'})
        ...
        async deleteElement(id) {
            await axios.delete(`/api/v1/references/grades/${id}`)
                .then(res => {
        ...
        async restoreElement(id, params) {
            await axios.get(`/api/v1/references/grades/restore/${id}`)
                .then(res => {
                    this.getElements({page: this.page, ...this.filterParams})
        ...
        async showModalForm(title, action, element = null) {
            ...
            await this.$nextTick()
            this.modal = document.getElementById('gradeForm')
            this.modalObj = new Modal(this.modal)
            this.formTitle = title + ' оценку'
