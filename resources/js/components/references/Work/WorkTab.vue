                ...
                <div class="table-responsive table-responsive-reference">
                    ...
                    <table class="table table-hover table-bordered table-sm componentTable bg-white">
                        ...
                        <tr>
                            ...
                            <th rowspan="2" class="text-center align-middle">Фон карточки занятия</th>
                        ...
                        <tr v-for="element in elements.data">
                            ...
                            <td>
                                <div class="work-color-div">
                                    <span class="work-color-span" :style="{'background-color': element.color}"></span>
                                    <span>
                                        {{ element.color }}
                                    </span>
                                </div>
...
export default {
    ...
    methods: {
        async getElements(params) {
            ...
            await axios.get('/api/v1/references/works', {params})
                .then(res => {
        ...
        async storeElement() {
            await axios.post('/api/v1/references/works', this.element)
                .then(res => {
                    this.modalObj.hide()
        ...
        async updateElement() {
            await axios.patch(`/api/v1/references/works/${this.element.id}`, this.element)
                .then(res => {
                    this.modalObj.hide()
                    this.getElements({page: this.page, ...this.filterParams})
                    this.$notify({title: this.saveMsg, type: 'success'})
        ...
        async deleteElement(id) {
            await axios.delete(`/api/v1/references/works/${id}`)
                .then(res => {
        ...
        async restoreElement(id) {
            await axios.get(`/api/v1/references/works/restore/${id}`)
                .then(res => {
                    this.getElements({page: this.page, ...this.filterParams})
        ...
        async showModalForm(title, action, element = null) {
            ...
            await this.$nextTick()
            this.modal = document.getElementById('workForm')
            this.modalObj = new Modal(this.modal)
            this.formTitle = title + ' тип работы'
