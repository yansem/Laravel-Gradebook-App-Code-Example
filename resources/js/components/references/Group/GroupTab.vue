        ...
        <div class="row">
            <div class="col-lg-12 mb-4">
                ...
                <div class="d-flex flex-wrap mb-2">
                    ...
                    <template v-for="(value, key, index) in filtersFormatted">
                ...
                <div class="table-responsive table-responsive-reference">
                    ...
                    <table class="table table-hover table-bordered table-sm componentTable bg-white">
                        ...
                        <tr>
                            ...
                            <th class="ref">Дата начала</th>
                            <th class="ref">Дата окончания</th>
                        ...
                        <tr>
                            ...
                            <td><input v-model="filters.group_start.value" type="date" class="form-control" onkeydown="return false"></td>
                            <td><input v-model="filters.group_end.value" type="date" class="form-control" onkeydown="return false"></td>
                        </tr>
                        </thead>
...
export default {
    ...
    computed: {
        ...
        filtersFormatted() {
            const filtersObj = _.cloneDeep(this.filters)
            if (filtersObj.group_start.value) {
                filtersObj.group_start.value = moment(filtersObj.group_start.value).format(`DD.MM.yyyy`)
            }
            if (filtersObj.group_end.value) {
                filtersObj.group_end.value = moment(filtersObj.group_end.value).format(`DD.MM.yyyy`)
            }
            return filtersObj
        },
    },
    ...
    methods: {
        async getElements(params) {
            ...
            await axios.get('/api/v1/references/groups', {params})
                .then(res => {
        ...
        async storeElement() {
            await axios.post('/api/v1/references/groups', this.element)
                .then(res => {
                    this.modalObj.hide()
        ...
        async updateElement() {
            await axios.patch(`/api/v1/references/groups/${this.element.id}`, this.element)
                .then(res => {
                    this.modalObj.hide()
                    this.getElements({page: this.page, ...this.filterParams})
                    this.$notify({title: this.saveMsg, type: 'success'})
        ...
        async deleteElement(id) {
            await axios.delete(`/api/v1/references/groups/${id}`)
                .then(res => {
        ...
        async restoreElement(id) {
            await axios.get(`/api/v1/references/groups/restore/${id}`)
                .then(res => {
                    this.getElements({page: this.page, ...this.filterParams})
        ...
        async showModalForm(title, action, element = null) {
            if (element !== null) {
                ...
                this.element.date_start = moment(element.date_start, 'DD.MM.YYYY').format('YYYY-MM-DD')
                this.element.date_end = moment(element.date_end, 'DD.MM.YYYY').format('YYYY-MM-DD')
            }
            this.isShowModal = true
            await this.$nextTick()
            this.modal = document.getElementById('groupForm')
            this.modalObj = new Modal(this.modal)
            this.formTitle = title + ' группу'
